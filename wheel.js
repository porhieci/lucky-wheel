...
var stud_arr = [];
students.forEach(e => {stud_arr.push(new Student(e['firstname'], e['lastname'], e['promo'], e['td_group'], e['tp_group'], e['pic'], e['prime']))});
...
document.addEventListener('keyup', event => {
    if (event.code === 'Space') {
        var nIntervId = setInterval((old, rd) => {
            ...
            // price
            var random = stud.prime ?? Math.ceil(Math.random()*100000) * 5000
